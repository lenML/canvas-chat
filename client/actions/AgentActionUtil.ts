...
const defaultRegistry = new Map<
  string,
  AgentActionUtilConstructor<BaseAgentAction>
>();

// Mode registry: actionType -> (mode -> util) (mode-specific overrides)
const modeRegistry = new Map<
  string,
  Map<string, AgentActionUtilConstructor<BaseAgentAction>>
>();
 ...
 */
export function registerActionUtil<
  T extends AgentActionUtilConstructor<BaseAgentAction>
>(util: T, options?: RegisterActionUtilOptions): T {
  ...
  if (forModes && forModes.length > 0) {
    ...
    for (const mode of forModes) {
      if (modeMap.has(mode)) {
        throw new Error(
          `Action util for ${util.type} already registered for mode ${mode}`
        );
      }
 ...
 */
export function getAgentActionUtilsRecordForMode(
  agent: TldrawAgent,
  mode: string
) {
  ...
  for (const [type, util] of defaultRegistry.entries()) {
    object[type as AgentAction["_type"]] = new util(
      agent
    ) as AgentActionUtil<AgentAction>;
  }
  ...
  for (const [type, modeMap] of modeRegistry.entries()) {
    ...
    if (modeUtil) {
      object[type as AgentAction["_type"]] = new modeUtil(
        agent
      ) as AgentActionUtil<AgentAction>;
    }
  }
