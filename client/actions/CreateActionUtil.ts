...
export const CreateActionUtil = registerActionUtil(
  class CreateActionUtil extends AgentActionUtil<CreateAction> {
    ...
    override applyAction(
      ...
      const result = convertPartialFocusedShapeToTldrawShape(
        editor,
        shapePartial,
        {
...
function getDefaultShape(
  shapeType: FocusedShape["_type"],
  complete: boolean
): Partial<TLShape> {
  const isGeo = shapeType in FOCUSED_TO_GEO_TYPES;
  const defaultShape = isGeo
    ? SHAPE_DEFAULTS.geo
    : SHAPE_DEFAULTS[shapeType as keyof typeof SHAPE_DEFAULTS] ??
      SHAPE_DEFAULTS.unknown;
  return complete ? defaultShape : { ...defaultShape, isLocked: true };
}

type BaseShapeDefaults = Pick<
  TLShape,
  "id" | "isLocked" | "opacity" | "rotation" | "meta"
>;
