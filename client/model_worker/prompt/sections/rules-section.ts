...
${flagged(
  flags.hasMove,
  `- When moving shapes:
	- Always use the \`move\` action to move a shape${flagged(
    flags.hasUpdate,
    ", never the `update` action"
  )}.`
)}
${flagged(
  flags.hasUpdate,
...
- Text shapes
	...
	- By default, the width of text shapes will grow to fit the text content${flagged(
    flags.hasScreenshotPart,
    ". Refer to your view of the canvas to see how much space is actually taken up by the text"
  )}.
...
- Colors
	- When specifying a fill, you can use \`background\` to make the shape the same color as the background${flagged(
    flags.hasScreenshotPart,
    ", which you'll see in your viewport"
...
${flagged(
  flags.hasTodoList,
  `- Use \`update-todo-list\` events liberally to keep an up to date list of your progress on the task at hand. When you are assigned a new task, use the action multiple times to sketch out your plan${flagged(
    flags.hasReview,
    ". You can then use the `review` action to check the todo list"
  )}.
...
${flagged(
  ...
  `- Carefully plan which action types to use. For example, the higher level events like ${[
    flags.hasDistribute && "`distribute`",
    flags.hasStack && "`stack`",
    flags.hasAlign && "`align`",
    flags.hasPlace && "`place`",
  ]
    .filter(Boolean)
    .join(", ")} can at times be better than the lower level events like ${[
    flags.hasCreate && "`create`",
    flags.hasUpdate && "`update`",
    flags.hasMove && "`move`",
  ]
    .filter(Boolean)
    .join(
...
${flagged(
  flags.hasUserViewportBoundsPart ||
    flags.hasAgentViewportBoundsPart ||
    flags.hasSetMyView,
