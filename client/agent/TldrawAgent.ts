...
import {
  AgentDebugFlags,
  AgentDebugManager,
} from "./managers/AgentDebugManager";
...
export class TldrawAgent {
  ...
  async preparePrompt(
    ...
    for (const promptPartType of availablePromptPartTypes) {
      ...
      if (!util)
        throw new Error(
          `Prompt part util not found for part type: ${promptPartType}`
        );
  ...
  async prompt(
    ...
    if (this.requests.isGenerating() && !nested) {
      throw new Error(
        "Agent is already prompting. Please wait for the current prompt to finish."
      );
    }
  ...
  schedule(input: AgentInput) {
    ...
    this._schedule({
      ...
      agentMessages: [
        ...scheduledRequest.agentMessages,
        ...(newRequest.agentMessages ?? []),
      ],
      userMessages: [
        ...scheduledRequest.userMessages,
        ...(newRequest.userMessages ?? []),
      ],
  ...
  private requestAgentActions(request: AgentRequest) {
    ...
    const promptHistoryItem: ChatHistoryPromptItem = {
      ...
      selectedShapes: this.editor
        .getSelectedShapes()
        .map((shape) =>
          convertTldrawShapeToFocusedShape(this.editor, structuredClone(shape))
        ),
    };
    ...
    const requestPromise = (async () => {
      ...
      try {
        for await (const action of this.streamAgentActions({
          prompt,
          signal,
        })) {
          if (cancelled) break;
          ...
          try {
            editor.run(
              () => {
                const actionUtilType = this.actions.getAgentActionUtilType(
                  action._type
                );
                const actionUtil = this.actions.getAgentActionUtil(
                  action._type
                );
                ...
                const { diff, promise } = this.actions.act(
                  transformedAction,
                  helpers
                );
