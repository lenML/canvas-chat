...
export class AgentLintManager extends BaseAgentManager {
  ...
  private getShapesWithGrowY(shapes: TLShape[]): TLShape[] {
    ...
    for (const shape of shapesWithGrowY) {
      const overlapsWithGeo = geoShapes.some((geoShape) =>
        this.shapesOverlap(shape, geoShape)
      );
      if (overlapsWithGeo) {
        result.push(shape);
      }
  ...
  private shapesOverlap(shapeA: TLShape, shapeB: TLShape): boolean {
    ...
    const polygonA = clipPathA
      ? intersectPolygonPolygon(
          pageTransformA.applyToPoints(clipPathA),
          verticesA
        )
      : verticesA;
    ...
    const polygonAInShapeBSpace = pageTransformB
      .clone()
      .invert()
