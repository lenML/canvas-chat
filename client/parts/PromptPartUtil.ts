 ...
 */
export function registerPromptPartUtil<
  T extends PromptPartUtilConstructor<BasePromptPart>
>(util: T): T {
...
export function getAllPromptPartUtils(): PromptPartUtilConstructor<PromptPart>[] {
  return Array.from(
    registry.values()
  ) as PromptPartUtilConstructor<PromptPart>[];
}
...
export function getPromptPartUtilsRecord(agent: TldrawAgent) {
  ...
  for (const util of registry.values()) {
    object[util.type as PromptPart["type"]] = new util(
      agent
    ) as PromptPartUtil<PromptPart>;
  }
  return object;
}
